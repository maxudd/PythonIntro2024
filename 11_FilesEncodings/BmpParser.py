...
https://uneex.org/LecturesCMC/PythonIntro2024/Homework_BmpParser
"""
...
if dib_size == 12:
    # print('ya old')
    width = abs(int.from_bytes(bytes(dib_bytes_list[0:2]),
                               "little",
                               signed=True))
    height = abs(int.from_bytes(bytes(dib_bytes_list[2:4]),
                                "little",
                                signed=True))
    bits_per_pixel = abs(int.from_bytes(bytes(dib_bytes_list[6:8]),
                                        "little",
                                        signed=True))
    comp_method = 0
    size = 0
elif dib_size in [16, 40, 52, 56, 64, 108, 124]:
    width = abs(int.from_bytes(bytes(dib_bytes_list[0:4]),
                               "little",
                               signed=True))
    height = abs(int.from_bytes(bytes(dib_bytes_list[4:8]),
                                "little",
                                signed=True))
    bits_per_pixel = abs(int.from_bytes(bytes(dib_bytes_list[10:12]),
                                        "little",
                                        signed=True))
    comp_method = int.from_bytes(bytes(dib_bytes_list[12:16]),
                                 "little")
    size = abs(int.from_bytes(bytes(dib_bytes_list[16:20]),
                              "little",
                              signed=True))
else:
